...
if (!inputFile) {
    console.error("Usage: node oracle_cluster3.mjs <input.json>");
    process.exit(1);
}
...
const results = cases.map(c => {
    try {
        ...
        const addRes = JSBI.add(xVal, yVal);
        const subRes = JSBI.subtract(xVal, yVal);

        return {
            status: 'OK',
            addSign: addRes.sign,
            addLen: addRes.length,
            addDigits: Array.from(addRes),
            subSign: subRes.sign,
            subLen: subRes.length,
            subDigits: Array.from(subRes)
        };
    } catch (e) {
